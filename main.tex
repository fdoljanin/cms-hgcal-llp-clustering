...
% Mark plots made from the simulated samples used in this report.
\newsavebox{\cmsplotbox}
\newcommand{\cmsplot}[2][]{%
  \begingroup
  \sbox{\cmsplotbox}{\includegraphics[#1]{#2}}%
  \usebox{\cmsplotbox}%
    \llap{\raisebox{\dimexpr\ht\cmsplotbox+0.1em\relax}{%
      \makebox[\wd\cmsplotbox][l]{%
        \bfseries\scriptsize CMS\enspace\normalfont\itshape Private Work}}}%
  \endgroup
}
...
\author{Frane Doljanin\\
  ...
  \small\textit{Supervisors: Bruno Alves and Marco Rovere, CERN, Experimental Physics Department}\\
  \small\href{mailto:bruno.alves@cern.ch}{\texttt{bruno.alves@cern.ch}}\quad
  \small\href{mailto:marco.rovere@cern.ch}{\texttt{marco.rovere@cern.ch}}}
...
\input{sections/02_displacement}
\input{sections/03_displaced_photon_gun}
\input{sections/04_metrics}
\input{sections/05_clue3d}
\input{sections/06_single_photon_performance}

\input{sections/07_two_photon_merging}
\input{sections/08_pileup_performance}
\input{sections/09_cluestering}
